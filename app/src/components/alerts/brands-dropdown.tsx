...
export function BrandsDropdown({
  ...
  return (
    <div
      className={cn(
        "absolute left-0 top-full z-50 mt-1 w-72 overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-lg",
        className,
      )}
    >
      ...
      <div className="px-2 py-3">
        <p className="mb-1.5 px-2 text-xs font-semibold tracking-wide text-zinc-400">
          Total Weekly Sales
        </p>
        ...
        <div className="space-y-0.5">
          {brands.map((brand) => (
            ...
            >
              ...
              <span className="flex-1 font-medium text-zinc-700">{brand.name}</span>

              <span className="font-semibold text-rose-500">
                {formatGapDollar(brand.gapDollar)}
              </span>
              <span className="text-zinc-400">{formatGapUnits(brand.gapUnits)}</span>
            </button>
          ))}
        </div>
      </div>
